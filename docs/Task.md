...
- [/] **審査提出**（ユーザー実施）
  ...
  - [ ] 審査通過待ち
- [/] **実機検証（動作およびUI確認）**（ユーザー実施/エージェント支援）
  - [x] USBデバッグ接続の確立 (完了)
  - [x] 実機のADB認識確認 (完了)
  - [x] 実機ビルドおよびインストール (完了)
  - [x] スキャン種別フィルタ（1D/2D）の反映 (合格)
  - [x] スキャン範囲（単一）の精度 (合格)
  - [x] スキャン範囲（複数）のリセット動作 (修正完了・再検証待ち)
  - [x] バリデーション検知の拡充 (JAN-8対応等) (修正完了・再検証待ち)
  - [x] Start/Stop文字検証 (メッセージ不一致の修正完了)
- [/] **バグ再修正と追加検証**
  - [x] 再修正計画の承認 (2026-02-01 23:49 承認済み)
  - [x] 検証用デモ画像 (demo_8~11) の生成 (完了)
  - [x] スキャン窓の完全リセット実装 (合格・動作確認済み)
  - [x] バリデーション検知の拡充 (JAN-8/Start-Stop等、エラーメッセージが「不正解」と誤表示されるため不合格・再修正中) (修正完了・再検証待ち)
  - [x] 実機カメラ・シャッター不具合の修正と復旧 (完了・再検証待ち)
