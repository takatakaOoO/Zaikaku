...
## フェーズ 7: ストア公開準備と製品化設定 [未着手]
- [ ] 本番用AdMob設定
  - [ ] AdMobコンソールでのアプリ・広告ユニット作成
  - [ ] 本番用App ID/ユニットIDへの差し替え
  - [ ] app-ads.txt の配置準備
- [ ] 法的・ドキュメント対応
  - [ ] 取扱説明書 (docs/USER_MANUAL.md) の作成
  - [ ] プライバシーポリシーの作成とアプリ内掲示
  - [ ] OSSライセンス表示機能の実装 (showLicensePage)
- [ ] Google Play Store 登録準備
  - [ ] リリース用署名鍵 (jks) の作成と設定
  - [ ] build.gradleへの署名構成追加
  - [ ] ストア掲載情報の作成（説明文、プライバシーポリシーURL等）
  - [ ] スクリーンショット、フィーチャーグラフィックの作成
- [ ] 最終ビルドと品質確認
  - [ ] `flutter build appbundle` によるリリースビルド生成
  - [ ] 難読化 (ProGuard/R8) の設定確認
  - [ ] 実機での最終回帰テスト
