...
## Phase 8: バグ修正と最終調整 [進行中]
- [x] バリデーション検知の修正
- [x] 実機カメラ・シャッター不具合の修正
- [x] 初期画面と遷移（戻るボタン）の復旧 (2026-02-02 実装完了)
- [ ] 実機（SC-52B）での全項目最終検証

### 変更履歴
- **2026-02-02**: 実機検証で判明した以下の不具合を修正。
    1.  **初期画面**: アプリ起動時に「ホーム画面」ではなく「スキャン画面」が表示される不具合を修正 (`initialLocation: '/'`).
    2.  **戻るボタン**: スキャン画面からホームに戻れない（アプリが終了する）問題を修正。`context.canPop()` チェックを追加し、履歴がない場合はホームへ遷移する安全策を実装。
    3.  **物理スキャン**: エラーダイアログを閉じた後にシャッターボタンが非活性のままとなる問題を修正。`ScanNotifier.clearError` での状態リセット漏れを解消。
    4.  **コンパイルエラー**: Releaseビルド時の `debugPrint` 参照エラーを修正。

### 検証項目（ユーザー依頼中）
実機（SC-52B）にて、以下の順序で動作をご確認ください。

1.  **起動フロー**: アプリ起動時に「ホーム画面」が表示されること。
2.  **画面遷移**: ホーム -> スキャン開始 -> 戻るボタン -> ホーム の遷移が正常であること。
3.  **物理スキャン復帰**: バーコード読み取りエラー（CDエラー等）発生後、「閉じる」を押すと即座にシャッターボタンが赤く（活性化）なり、再スキャンが可能であること。
